...
namespace VSEngine2
{
	...
	public:
		...
		/****************************inline********************************************/
		//得到控制点
		inline VSVector3 GetControlPoint(unsigned int U,unsigned int V)const;
		//改变控制点
		inline bool ChangeControlPoint(unsigned int U,unsigned int V,const VSVector3& Value);
		//得到控制点指针
		inline const VSVector3 *GetControlPoint()const;
		//获得控制点个数
		inline unsigned int GetControlPointNum()const;
		//获得行数
