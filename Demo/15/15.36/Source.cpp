...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	VSSkeletonActor * pSKActor1 = (VSSkeletonActor *)VSWorld::ms_pWorld->CreateActor(_T("MonsterPhoneWolrdOffset.SKMODEL"), VSVector3(0, 0, 0), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
...
bool VSDemoWindowsApplication::PostUpdate()
{
	...
	pMaterialInstance0->SetShaderValue(_T("ExtendLength"), &ExtendLength, sizeof(ExtendLength));
	pMaterialInstance1->SetShaderValue(_T("ExtendLength"), &ExtendLength, sizeof(ExtendLength));
	return true;
}
...
bool VSDemoWindowsApplication::OnDraw()
{
	...
	VSRenderer::ms_pRenderer->DrawText(300, 640, VSDWCOLORABGR(255, 255, 255, 255), _T("Use Key '+' And '-' to Change ExtendLength"));
	VSRenderer::ms_pRenderer->DrawText(300, 660, VSDWCOLORABGR(255, 255, 255, 255), _T("ExtendLength:%f"), ExtendLength);
	return true;
}
