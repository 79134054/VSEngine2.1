...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	VSPostEffectSetR * pGray = VSResourceManager::LoadASYNResource<VSPostEffectSet>(_T("PostEffect_Material_Gray_Bloom.POSTEFFECT"), false);
...
bool VSDemoWindowsApplication::OnDraw()
{
	...
	VSRenderer::ms_pRenderer->DrawText(300, 660, VSDWCOLORABGR(255, 255, 255, 255), _T("Use Key 'M' to Switch Material PostEffect"));
	VSRenderer::ms_pRenderer->DrawText(300, 680, VSDWCOLORABGR(255, 255, 255, 255), _T("Use Key 'B' to Switch Bloom PostEffect"));
	return true;
}
...
void VSDemoWindowsApplication::OnKeyDown(unsigned int uiKey)
{
	...
	else if (uiKey == VSEngineInput::BK_B)
	{

		VSViewFamily * pViewFamily = VSSceneManager::ms_pSceneManager->GetViewFamily(_T("WindowUse"));
		VSSceneRenderMethod * pRM = pViewFamily->m_pSceneRenderMethod;
		VSPostEffectSet * pPESet = pRM->GetPostEffectSet();
		static VSUsedName PEBloomName = _T("Bloom");
		VSPostEffectFunction * PEBloom = pPESet->GetPEFunctionFromShowName(PEBloomName);
		PEBloom->m_bEnable = !PEBloom->m_bEnable;
	}
