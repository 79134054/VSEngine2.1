...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	VSString AnimName[] = { _T("Idle"), _T("Walk"), _T("Attack"),_T("Attack2"), _T("Pos"), _T("AddtiveAnim")};

	for (unsigned int i = 0; i < 11; i++)
	{
		for (unsigned int j = 0; j < 11; j++)
		{
			unsigned int s = VSRand() % 6;
			VSSkeletonActor * pSKActor = (VSSkeletonActor *)VSWorld::ms_pWorld->CreateActor(_T("NewMonsterWithAnim.SKMODEL"), VSVector3(j * 400.0f - 1500.0f, 0, i * 400.0f + 1000.0f), VSMatrix3X3::ms_Identity, VSVector3::ms_One, m_pTestMap);
