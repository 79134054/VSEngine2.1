//播放Demo14.20带Morph模型。读者可以通过“+”和“-”来调节混合参数，来播放Morph脸部表情动画
...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	pSTActor = (VSStaticActor *)VSWorld::ms_pWorld->CreateActor(_T("NewFaceMorph.STMODEL"), VSVector3(0.0f, 0.0f, 0.0f), VSMatrix3X3::ms_Identity, VSVector3(3.0f, 3.0f, 3.0f), m_pTestMap);
...
bool VSDemoWindowsApplication::PostUpdate()
{
	...
	static VSUsedName MorphName = _T("F");
	pSTActor->GetTypeNode()->SetMorphData(MorphName, m_fPencent);
	return true;
}
...
bool VSDemoWindowsApplication::OnDraw()
{
	...
	VSRenderer::ms_pRenderer->DrawText(300, 640, VSDWCOLORABGR(255,255,255,255), _T("Use Key '+' And '-' to Change SmoothMorphBlendParamter"));
	VSRenderer::ms_pRenderer->DrawText(300, 660, VSDWCOLORABGR(255, 255, 255, 255), _T("SmoothMorphBlendParamter:%f"), m_fPencent);
	return true;
}
