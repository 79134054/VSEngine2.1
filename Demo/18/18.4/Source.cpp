//展示静态模型 材质变量实例 动态instance 仅支持DX11
...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	m_pCameraActor->GetTypeNode()->AddController(m_p1stCameraController);
 	VSVector3 CameraPos(0.0f, 250.0f, -300.0f);
	...
	for (unsigned int y = 0; y < 1; y++)
	{
		for (int z = -RENDER_NUM; z <= RENDER_NUM; z++)
		{
			...
			{
				VSREAL fRand = VSFRand() + 1.0f;
				VSVector3 Scale(fRand, fRand, fRand);
				...
				VSStaticActor* pActor = (VSStaticActor*)VSWorld::ms_pWorld->CreateActor(_T("NewInstanceTestStone.STMODEL"), VSVector3(x * 800.0f, y * 800.0f, z * 800.0f), RandRotator, Scale, m_pTestMap);
				VSMaterialInstance* pMaterialInstance = pActor->GetTypeNode()->GetUseMaterialInstance(0);
				VSColorRGBA InstanceColor(VSFRand(), VSFRand(), VSFRand(),1.0f);
				pMaterialInstance->SetShaderValue(_T("DiffuseColor"), &InstanceColor, sizeof(InstanceColor));
			}
		}

	}
	VSStaticActor* pStaticActor = (VSStaticActor*)VSWorld::ms_pWorld->CreateActor(_T("NewOceanPlane.STMODEL"), VSVector3(0, -100, 0), VSMatrix3X3::ms_Identity, VSVector3(100.0f, 100.f, 100.0f), m_pTestMap);
