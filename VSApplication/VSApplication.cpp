...
bool VSApplication::CreateStreamingManager()
{
	...
	{
		VSStreamingManager* pStreamingManager = VS_NEW VSStreamingManager();
...
{
	VSMAC_DELETE(VSStreamingManager::ms_pStreamingManager);
	return true;
}
...
bool VSApplication::Run()
{
	...
	if (VSTimer::ms_pTimer)
	{
		...
		ADD_ONLYTIME_PROFILE(FPS, (VSREAL)fFPS)
	}
	if (VSRenderer::ms_pRenderer)
	{
		...
		VSResourceManager::DelayUpdate((VSREAL)fTime);
...
#ifdef WINDOWS_PLATFORM
DWORD VSWindowApplication::ms_WindowKeyToVS[] =
{
...
bool VSWindowApplication::PreInitial()
{
	...
	m_uiAnisotropy = 1;
	m_uiMultisample = VSRenderer::MS_NONE;
	m_Tile = _T("VSEngine2.0 by azhe --- Demo ") + VSRenderer::GetRenderTypeString(m_uiRenderAPIType);
#ifdef _WIN64
	m_Tile += _T(" x64");
#else
	m_Tile += _T(" x86");
...
bool VSConsoleApplication::CreateDx11()
{
	...
	{
		VSDX11Renderer * pRenderer =
			VS_NEW VSDX11Renderer(NULL, 0, 0,
